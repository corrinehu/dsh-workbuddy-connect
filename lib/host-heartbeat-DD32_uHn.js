...
*/
function domesticDesktopAuthCandidates() {
	const home = homedir();
...
function clientUserAgent(credential) {
...
var WorkBuddyUpstreamClient = class {
	...
	async fetchModels(credential) {
		...
		const path = global ? "/v3/config" : "/console/enterprises/personal/models";
		const response = await fetch(`${chatBase(credential)}${path}`, {
			headers: {
				...
				...global ? {
					...
					...credential.enterpriseId === void 0 || credential.enterpriseId === "" ? {} : { "X-Enterprise-Id": credential.enterpriseId },
