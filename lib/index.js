...
function apply(ctx, config) {
	...
	const accountEntries = (config.accounts ?? []).filter((key) => key.trim() !== "").map((key) => ({ key: key.trim() }));
	const multiAccount = accountEntries.length > 0;
	const store = multiAccount ? new WorkBuddyAccountManager({
	...
	shim.ready.then(() => {
		...
		try {
			...
			else for (const entry of accountEntries) registerOne(`${WORKBUDDY_PROVIDER}:${entry.key}`, `WorkBuddy · ${entry.key}`, entry.key);
			try {
				ctx.effect(() => () => {
		...
		(async () => {
			try {
				...
				if (store instanceof WorkBuddyAccountManager) {
					const key = config.defaultAccount ?? accountEntries[0]?.key;
					if (key !== void 0) credential = await store.resolve(key);
				} else credential = await store.current();
