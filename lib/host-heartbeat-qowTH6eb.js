...
const WORKBUDDY_AUTH_DIRNAME = ".workbuddy-auth";
...
function ownDocument(credential, key) {
	return {
...
var WorkBuddyAccountManager = class {
	...
	desktopStore() {
		return new WorkBuddyCredentialStore({
			...this.desktopPathOverride === void 0 ? {} : { desktopPath: this.desktopPathOverride },
	...
	*/
	async importFromDesktop(key, options) {
		const trimmed = key.trim();
		if (trimmed === "") throw new Error("workbuddy: account key must not be empty");
		if ((await this.listAccounts()).includes(trimmed) && options?.force !== true) throw new Error(`workbuddy: account "${trimmed}" already exists; pass force to overwrite`);
		const credential = await this.desktopStore().current();
		if (credential === void 0) throw new Error("workbuddy: no signed-in WorkBuddy account found in the desktop app; sign in there first");
		await this.storeFor(trimmed).seedOwn(credential);
		return this.accountInfo(trimmed);
	}
	/** Live sign-in summary for one account, or signed-out when absent/broken. */
	async statusOf(key) {
		if (!(await this.listAccounts()).includes(key)) return {
			...this.accountInfo(key),
			state: "signed-out"
		};
		const summary = await this.storeFor(key).status();
		if (summary.state !== "signed-in") return {
			...this.accountInfo(key),
			state: "signed-out"
		};
		return {
			...this.accountInfo(key),
			state: "signed-in",
			...summary.nickname === void 0 ? {} : { nickname: summary.nickname },
			...summary.domain === void 0 || summary.domain === "" ? {} : { domain: summary.domain },
			...summary.expiresAtMs === void 0 ? {} : { expiresAtMs: summary.expiresAtMs },
			...summary.refreshExpiresAtMs === void 0 ? {} : { refreshExpiresAtMs: summary.refreshExpiresAtMs },
	...
	async resolve(key) {
		if (!(await this.listAccounts()).includes(key)) throw new Error(`workbuddy: no such account "${key}"; import it with "dsh-workbuddy-connect import ${key}"`);
...
const WORKBUDDY_CONNECT_VERSION = "0.3.0";
//#endregion
...
//#endregion
export { parseWorkBuddyAuth as C, keyHash as S, workbuddyOwnAuthPath as T, WORKBUDDY_AUTH_FILE_ENV as _, readHostHeartbeat as a, defaultDesktopAuthCandidates as b, WORKBUDDY_CONNECT_VERSION as c, normalizeCredits as d, prepareChatBody as f, WORKBUDDY_AUTH_FILENAME as g, WorkBuddyCatalog as h, processStartTimeMs as i, WorkBuddyUpstreamClient as l, FALLBACK_WORKBUDDY_MODELS as m, clearHostHeartbeat as n, workbuddyHostHeartbeatPath as o, regionOf as p, isHeartbeatProcessAlive as r, writeHostHeartbeat as s, WORKBUDDY_HOST_HEARTBEAT_FILENAME as t, classifyUpstreamError as u, WorkBuddyAccountManager as v, workbuddyAccountDir as w, defaultDesktopAuthPath as x, WorkBuddyCredentialStore as y };
